...
import { Paperclip, Link2, BarChart3, MapPin, Smile, Globe, Lock, Users, X, Loader2, Navigation } from 'lucide-react';
...
export const CreatePostModal = ({ open, onOpenChange, onPostCreated, organizationId }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Match EditPostModal: non-empty placeholder so media-only / poll-only posts pass API validation when body text is empty.
      ...
      // Skip getPostById when no poll/hashtags – use create response for faster UI
      const fullPost = (hasPoll || hasHashtags) ? await getPostById(post.id) : post;
  ...
  const handleClose = () => {
    if (content.trim() || selectedFiles.length > 0 || poll || linkUrl.trim() || location.trim()) {
        ...
        <DialogHeader className="p-4 pb-0">
          <div className="flex items-center justify-between">
            ...
            <Select value={visibility} onValueChange={setVisibility}>
              <SelectTrigger className="w-auto h-8 gap-2 text-xs">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="public">
                  <div className="flex items-center gap-2">
                    ...
                    <span><LangText path="posts.public"  /></span>
                  </div>
                </SelectItem>
                <SelectItem value="followers">
                  <div className="flex items-center gap-2">
                    <Users className="w-3 h-3" />
                    <span><LangText path="people.followers"  /></span>
                  </div>
                </SelectItem>
                <SelectItem value="private">
                  <div className="flex items-center gap-2">
                    <Lock className="w-3 h-3" />
                    <span><LangText path="posts.only_me"  /></span>
                  </div>
                </SelectItem>
              </SelectContent>
            </Select>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="relative">
                <Textarea
                  ...
                  className="min-h-[120px] resize-none border-none bg-transparent p-0 focus-visible:ring-0 text-base placeholder:text-muted-foreground"
                  value={content}
                  onChange={(e) => {
                    clearModerationError();
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1">
              ...
              <Popover open={showLinkOpen} onOpenChange={setShowLinkOpen}>
                ...
                <PopoverContent className="w-80 max-h-[min(70vh,320px)] overflow-y-auto rounded-xl border-border shadow-lg" align="start" side="top" sideOffset={8} collisionPadding={16}>
              ...
              <Popover open={showLocationOpen} onOpenChange={handleLocationPopoverOpenChange}>
                ...
                >
                  ...
                  <Button
                    type="button"
                    variant="outline"
                    className="w-full justify-start gap-2 mt-3 shrink-0"
                  ...
                  <Button
                    type="button"
                    size="sm"
                    className="mt-3 w-full rounded-lg shrink-0"
                    onClick={() => {
            ...
            <div className="flex items-center gap-3">
              ...
              <Button
                ...
                className="px-6"
              >
                {isSubmitting ? toastT('common.publishing') : toastT('common.publish')}
              </Button>
            </div>
          </div>
        </div>
